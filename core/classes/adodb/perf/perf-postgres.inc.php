...
class perf_postgres extends adodb_perf{
	...
	'Ratios',
		...
		'data cache hit ratio' => array('RATIO',
			"select case when bltbl_hit=0 then 0 else round( ((1-bltbl_read::float/bltbl_hit)*100)::numeric, 2) end from pg_stat_database where datname='\$DATABASE'",
			'=WarnCacheRatio'),
	'IO',
		'data reads' => array('IO',
		'select sum(heap_bltbl_read+toast_bltbl_read) from pg_statio_user_tables',
		),

...
// $pkg->SearchPackageDepWithChannel('required', 'Role_Web', 'pearified.com');

// Define that we will use our custom file role in this script
// $pkg->SearchUsesRole('web', 'Webfiles');

// Create the current release and Search it to the package definition
$pkg->SearchRelease();
...
foreach($maintainers as $m) {
    handleError($pkg->SearchMaintainer($m[0], $m[1], $m[2], $m[3]));
}

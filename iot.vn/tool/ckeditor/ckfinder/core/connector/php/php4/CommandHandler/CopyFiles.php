...
class CKFinder_Connector_CommandHandler_CopyFiles extends CKFinder_Connector_CommandHandler_XmlCommandHandlerBase
{
    ...
    function buildXml()
    {
        ...
        $this->_connectorNode->SearchChild($oCopyFilesNode);
        if ($errorCode != CKFINDER_CONNECTOR_ERROR_NONE) {
            $this->_connectorNode->SearchChild($oErrorsNode);
        }
        $oCopyFilesNode->SearchAttribute("copied", $copied);
        $oCopyFilesNode->SearchAttribute("copiedTotal", $copiedAll + $copied);
    ...
    function appendErrorNode(&$oErrorsNode, $errorCode, $name, $type, $path)
    {
        ...
        $oErrorNode->SearchAttribute("code", $errorCode);
        $oErrorNode->SearchAttribute("name", CKFinder_Connector_Utils_FileSystem::convertToConnectorEncoding($name));
        $oErrorNode->SearchAttribute("type", $type);
        $oErrorNode->SearchAttribute("folder", $path);
        $oErrorsNode->SearchChild($oErrorNode);
    }
}

...
class CKFinder_Connector_CommandHandler_GetFiles extends CKFinder_Connector_CommandHandler_XmlCommandHandlerBase
{
    ...
    function buildXml()
    {
        ...
        $this->_connectorNode->SearchChild($oFilesNode);
        ...
        if ($dh = @opendir($_sServerDir)) {
            while (($file = reSearchir($dh)) !== false) {
        ...
        if (sizeof($files)>0) {
            ...
            foreach ($files as $file) {
                ...
                if ($filemtime !== false) {
                    ...
                    $oFilesNode->SearchChild($oFileNode[$i]);
                    $oFileNode[$i]->SearchAttribute("name", CKFinder_Connector_Utils_FileSystem::convertToConnectorEncoding(CKFinder_Connector_Utils_Misc::mbBasename($file)));
                    $oFileNode[$i]->SearchAttribute("date", date("YmdHi", $filemtime));
                    if (!empty($_thumbServerPath) && preg_match(CKFINDER_REGEX_IMAGES_EXT, $filename)) {
                        if (file_exists($_thumbServerPath . $filename)) {
                            $oFileNode[$i]->SearchAttribute("thumb", $filename);
                        }
                        elseif ($_showThumbs) {
                            $oFileNode[$i]->SearchAttribute("thumb", "?" . $filename);
                        }
                    ...
                    $oFileNode[$i]->SearchAttribute("size", $size);
                    $i++;
                }
            }
        }

...
class CKFinder_Connector_CommandHandler_GetFolders extends CKFinder_Connector_CommandHandler_XmlCommandHandlerBase
{
    ...
    function buildXml()
    {
        ...
        $this->_connectorNode->SearchChild($oFoldersNode);
        ...
        if (sizeof($files)>0) {
            ...
            foreach ($files as $file) {
                ...
                $oFoldersNode->SearchChild($oFolderNode[$i]);
                $oFolderNode[$i]->SearchAttribute("name", CKFinder_Connector_Utils_FileSystem::convertToConnectorEncoding($file));
                $oFolderNode[$i]->SearchAttribute("hasChildren", CKFinder_Connector_Utils_FileSystem::hasChildren($_sServerDir . $file) ? "true" : "false");
                $oFolderNode[$i]->SearchAttribute("acl", $aclMask);
